...
class IO {
    constructor(wiki) {
        this._jar = http.jar();
        this._wiki = wiki;
        this._url = util.wikiUrl(wiki);
    }
    _request(method, url, qs, transform, body) {
        return http({
            headers: {
    ...
    services(httpMethod, service, resource, args, qs, transform, body) {
        ...
        return this._request(
            httpMethod,
            `https://services.wikia.com/${service}${resource}`,
            qs,
            transform,
            body
        );
